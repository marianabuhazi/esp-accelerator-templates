...
template <size_t _DMA_WIDTH_> class esp_accelerator : public sc_module {
  public:
    // Input ports
    ...
    esp_accelerator(const sc_module_name &name) :
        sc_module(name), clk("clk"), rst("rst"), acc_done("acc_done"),
        dma_read_chnl("dma_read_chnl"), dma_read_ctrl("dma_read_ctrl"),
